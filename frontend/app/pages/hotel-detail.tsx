...
export default function HotelDetail({ hotel }: Props) {
  ...
  const total = room.price * Math.max(1, nights);
                ...
                {hotel.rooms.map((r) => (
                  ...
                  >
                    <div className="flex justify-between items-start flex-wrap gap-3 mb-3">
                      <div>
                        <h3 className={`text-base font-semibold mb-1 ${selectedRoom === r.id ? "text-gold-400" : "text-slate-100"}`}>
                          {r.name}
                        </h3>
          ...
          <div className="w-full xl:w-96 shrink-0">
            <div className="glass p-6 xl:sticky xl:top-24 animate-fade-in">
              ...
              <div className="flex items-baseline gap-1.5 mb-6">
                <span className="text-gold-500 text-3xl font-black">LKR {room.price}</span>
              ...
              <div className="flex flex-col gap-4 mb-5">
                ...
                <div>
                  ...
                  <select value={guests} onChange={(e) => setGuests(+e.target.value)} className={`${inputCls} cursor-pointer appearance-none`}>
              ...
              <button
                ...
                className="w-full py-4 rounded-full bg-gradient-to-r from-gold-500 to-gold-400 text-navy-900 font-bold text-base shadow-xl shadow-gold-500/30 hover:shadow-gold-500/50 hover:-translate-y-0.5 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:translate-y-0 cursor-pointer"
              >
                {!checkIn || !checkOut ? "Select Dates to Book" : `Reserve — LKR ${Math.round(total * 1.12)}`}
              </button>
